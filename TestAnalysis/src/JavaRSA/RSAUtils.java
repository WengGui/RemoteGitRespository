...
*//* public abstract class RSAUtils { 
   ...
   *//** 
   ...
   public static synchronized KeyPair generateKeyPair() { 
       try { 
    	   byte[] dateBytes = DateFormatUtils.format(new Date(), "yyyyMMdd").getBytes();
    	   SecureRandom random = new SecureRandom(dateBytes);
   ...
   *//** 
   ...
   public static String decryptString(String encrypttext) { 
        ...
        * 
        * HttpServletRequest request = ServletActionContext.getRequest();//('|\||"|or)
		RSAPrivateKey privKey = (RSAPrivateKey) request.getSession().getAttribute("privKey");
        
       HttpServletRequest request = ServletActionContext.getRequest();//('|\||"|or)
       PrivateKey pKey=(PrivateKey) request.getSession().getAttribute("privKey");
     //  KeyPair keyPair = getKeyPair(); 
       try { 
           byte[] en_data = Hex.decodeHex(encrypttext.toCharArray()); 
          // byte[] data = decrypt((RSAPrivateKey)keyPair.getPrivate(), en_data); 
           byte[] data = decrypt((RSAPrivateKey)pKey, en_data); 
           return new String(data); 
       } catch(NullPointerException ex) { 
           LOGGER.error("keyPair cannot be null."); 
       } catch (Exception ex) { 
   ...
   *//** 返回已初始化的默认的公钥。*//*
   public static RSAPublicKey getDefaultPublicKey() { 
       ...
       if(keyPair != null) { 
    	   HttpServletRequest request = ServletActionContext.getRequest();//('|\||"|or)
    	   request.getSession().setAttribute("privKey", keyPair.getPrivate());
